...
from PySide6.QtGui import QPainter, QBrush, QColor, QAction, QPainterPath
...
class ChessBoard(QGraphicsView):
    ...
    def wheelEvent(self, event):
        """Permite navegar por las jugadas con la rueda del ratón"""
    ...
    def update_board(self):
        ...
        if self.highlighted_square is not None:
            ...
            pen = QPen(QColor(0, 220, 255), 4)
            self.scene.addRect(c * self.square_size + 2, r * self.square_size + 2,
        ...
        if self.selected_square is not None:
            for move in self.board.legal_moves:
                if move.from_square == self.selected_square:
                    ...
                    margin, size = self.square_size * 0.35, self.square_size * 0.3
                    self.scene.addEllipse(c * self.square_size + margin, r * self.square_size + margin, 
                                          size, size, Qt.NoPen, QBrush(QColor(0, 0, 0, 60)))
        ...
        self.draw_move_arrow(self.engine_move, QColor(0, 120, 255, 100)) # Azul más suave
        self.draw_move_arrow(self.hover_move, QColor(255, 0, 0, 60))    # Rojo muy sutil
