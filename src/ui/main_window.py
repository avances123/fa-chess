...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.toolbar_ana = QToolBar(); self.toolbar_ana.setMovable(False)
        self.setup_toolbar(self.toolbar_ana)
        board_container_layout.addWidget(self.toolbar_ana)
        
        # Layout horizontal para Barra de Eval + Tablero
        board_eval_layout = QHBoxLayout()
        board_eval_layout.setSpacing(5)
        
        self.eval_bar = QProgressBar()
        self.eval_bar.setOrientation(Qt.Vertical)
        self.eval_bar.setRange(-1000, 1000) # De -10 a +10 centipawns
        self.eval_bar.setValue(0)
        self.eval_bar.setTextVisible(False)
        self.eval_bar.setFixedWidth(15)
        self.eval_bar.setStyleSheet("""
            QProgressBar { border: 1px solid #777; background-color: #333; }
            QProgressBar::chunk { background-color: #eee; }
        """)
        self.eval_bar.setVisible(False)
        
        self.board_ana = ChessBoard(self.board, self)
        self.board_ana.color_light, self.board_ana.color_dark = self.def_light, self.def_dark
        
        board_eval_layout.addWidget(self.eval_bar)
        board_eval_layout.addWidget(self.board_ana)
        board_container_layout.addLayout(board_eval_layout)
        
        ana_layout.addWidget(board_container)
        
        panel_ana = QWidget(); p_ana_layout = QVBoxLayout(panel_ana)
        
        # --- SECCIÓN INFORMACIÓN (Apertura + Evaluación) ---
        info_box = QWidget()
        ...
        self.label_eval.setStyleSheet("font-weight: bold; font-size: 14px; color: #d32f2f; margin-left: 10px;")
        
        info_layout.addWidget(self.label_apertura, 1)
        ...
        self.tree_ana.setFont(table_font); self.tree_ana.cellClicked.connect(self.on_tree_cell_click)
        ...
        self.btn_search = QPushButton("🔍 Filtrar Partidas"); self.btn_search.clicked.connect(self.open_search)
        db_sidebar.addWidget(self.btn_search)
        
        self.label_db_stats = QLabel("[0/0]"); self.label_db_stats.setAlignment(Qt.AlignCenter)
        db_sidebar.addWidget(self.label_db_stats)
        db_sidebar.addWidget(self.progress); db_layout.addLayout(db_sidebar, 1)
        ...
        self.db_table.setFont(table_font); self.db_table.itemDoubleClicked.connect(self.load_game_from_list)
    ...
    def toggle_engine(self, checked):
        ...
        if checked:
            ...
            self.engine_worker.update_position(self.board.fen())
            ...
            self.board_ana.set_engine_move(None)

    def on_engine_update(self, eval_str, best_move, mainline):
        self.label_eval.setText(eval_str)
        
        # Dibujar flecha del motor en el tablero
        if mainline:
            # La primera jugada de la mainline es la mejor
            # Necesitamos pasarla al tablero (board_ana)
            # Primero la convertimos a objeto Move si es necesario, o pasamos el texto SAN
            # Por simplicidad, el motor ya nos da el mejor_movimiento en UCI en el segundo parámetro
            self.board_ana.set_engine_move(best_move)
        else:
            self.board_ana.set_engine_move(None)

        # Actualizar barra de eval
        try:
            if "M" in eval_str:
    ...
    def setup_toolbar(self, toolbar):
        ...
        actions = [
            (qta.icon('fa5s.step-backward'), self.go_start, "Inicio (Home)"),
            (qta.icon('fa5s.chevron-left'), self.step_back, "Anterior (Izquierda)"),
            (qta.icon('fa5s.chevron-right'), self.step_forward, "Siguiente (Derecha)"),
            (qta.icon('fa5s.step-forward'), self.go_end, "Final (End)"),
            (None, None, None),
            (qta.icon('fa5s.retweet'), self.flip_boards, "Girar Tablero (F)"),
        ]
        for icon, func, tip in actions:
            if icon is None: toolbar.addSeparator()
            else:
                action = toolbar.addAction(icon, ""); action.triggered.connect(func); action.setToolTip(tip)
        
        # Botón del motor con un icono de microchip moderno
        self.action_engine = QAction(qta.icon('fa5s.microchip', color='#444'), "", self)
        self.action_engine.setCheckable(True)
        self.action_engine.setToolTip("Activar/Desactivar Motor (E)")
        self.action_engine.triggered.connect(self.toggle_engine)
        toolbar.addAction(self.action_engine)

        right_spacer = QWidget(); right_spacer.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        toolbar.addWidget(right_spacer)

    def toggle_engine_shortcut(self):
        self.action_engine.toggle()
        self.toggle_engine(self.action_engine.isChecked())

    def add_to_clipbase(self):
        line_uci = " ".join([m.uci() for m in self.full_mainline])
        game_data = {
            "id": int(time.time()), "white": "Jugador Blanco", "black": "Jugador Negro",
            "w_elo": 2500, "b_elo": 2500, "result": "*", 
            "date": datetime.now().strftime("%Y.%m.%d"),
            "event": "Análisis Local", "line": line_uci, "full_line": line_uci
        }
    ...
    def delete_selected_game(self):
        ...
        if row >= 0:
            ...
            if self.db.delete_game(self.db.active_db_name, game_id):
                self.refresh_db_list()
                self.statusBar().showMessage("Partida eliminada", 2000)

    def on_db_table_context_menu(self, pos):
        menu = QMenu()
        copy_action = QAction("📋 Copiar a Clipbase", self); copy_action.triggered.connect(self.copy_to_clipbase); menu.addAction(copy_action)
        edit_action = QAction("📝 Editar Datos Partida", self); edit_action.triggered.connect(self.edit_selected_game); menu.addAction(edit_action)
        ...
        if not is_readonly or self.db.active_db_name == "Clipbase":
            menu.addSeparator()
            del_action = QAction("❌ Eliminar Partida", self); del_action.triggered.connect(self.delete_selected_game); menu.addAction(del_action)
    ...
    def on_db_list_context_menu(self, pos):
        ...
        menu = QMenu()
        remove_action = QAction("❌ Quitar de la lista", self); remove_action.triggered.connect(lambda: self.remove_database(item)); menu.addAction(remove_action)
    ...
    def remove_database(self, item):
        ...
        if self.db.remove_database(name):
            ...
            if self.db.active_db_name == "Clipbase": self.db_list_widget.setCurrentRow(0)
    ...
    def copy_to_clipbase(self):
        ...
        if row >= 0:
            ...
            if game_data:
                ...
                cols = self.db.dbs["Clipbase"].columns
                clean_data = {k: game_data[k] for k in cols}
    ...
    def edit_selected_game(self):
        ...
        target_db = self.db.active_db_name
        if is_readonly and target_db != "Clipbase":
            ...
            if ret == QMessageBox.Yes:
                self.copy_to_clipbase(); target_db = "Clipbase"
                self.refresh_db_list(); row = self.db_table.rowCount() - 1
                game_id = self.db_table.item(row, 0).data(Qt.UserRole)
            else: return
        ...
        if current_row:
            ...
            if dialog.exec_():
                if self.db.update_game(target_db, game_id, dialog.get_data()):
                    self.refresh_db_list(); self.statusBar().showMessage("Partida actualizada", 2000)
    ...
    def load_parquet(self, path):
        ...
        self.save_config(); self.refresh_db_list()
    ...
    def open_settings(self):
        ...
        if dialog.exec_():
            ...
            self.board_ana.color_light = light; self.board_ana.color_dark = dark
            self.board_ana.update_board(); self.save_config()

    def resizeEvent(self, event):
        h = self.centralWidget().height() - 40
        self.board_ana.setFixedWidth(h); super().resizeEvent(event)
    ...
    def switch_db(self, row):
        if row < 0: return
        self.db.active_db_name = self.db_list_widget.item(row).text()
        self.refresh_db_list(); self.update_stats()
    ...
    def refresh_db_list(self, df_to_show=None):
        ...
        if df_to_show is None:
            total = df.height; self.label_db_stats.setText(f"[{total}/{total}]")
            self.search_criteria = {"white": "", "black": "", "min_elo": "", "result": "Cualquiera"}
    ...
    def update_ui(self):
        ...
        if hasattr(self, 'engine_worker') and self.engine_worker.isRunning():
            self.engine_worker.update_position(self.board.fen())
    ...
    def on_tree_cell_click(self, row, column):
        table = self.sender(); it = table.item(row, 0); uci = it.data(Qt.UserRole)
        ...
        if os.path.exists(CONFIG_FILE):
            with open(CONFIG_FILE, "r") as f:
                try:
                    ...
                    if colors:
                        self.def_light = colors.get("light", self.def_light); self.def_dark = colors.get("dark", self.def_dark)
                except: pass
    ...
    def update_stats(self):
        ...
        res = self.db.get_stats_for_position(line_uci, self.board.turn == chess.WHITE)
        if res is None: self.tree_ana.setRowCount(0); return
        
        table = self.tree_ana; table.setRowCount(res.height)
        ...
        for i, r in enumerate(res.rows(named=True)):
            ...
            it_move = QTableWidgetItem(san); it_move.setData(Qt.UserRole, r["uci"]); table.setItem(i, 0, it_move)
            count = r["c"]; w, b, d = r["w"], r["b"], r["d"]
            score = (w + 0.5 * d) / count if is_white else (b + 0.5 * d) / count
